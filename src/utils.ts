...
export function normalizeDate(val: any): string {
  try {
    if (!val) return "";
    // Si viene como array (el error que causaba todo), agarramos solo el primer valor
    let str = Array.isArray(val) ? String(val) : (typeof val === 'string' ? val : JSON.stringify(val));
...
export function calculateEndDate(startDateStr: any, plan: PlanType): string {
  ...
  try {
    ...
    if (details) {
      if (details.durationDays) date.setDate(date.getDate() + details.durationDays);
      else if (details.durationMonths) date.setMonth(date.getMonth() + details.durationMonths);
    }
