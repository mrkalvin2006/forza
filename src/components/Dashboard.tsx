...
import { Search, Plus, Edit2, Trash2, MessageCircle, LogOut, Users, CalendarClock, Loader2, AlertTriangle } from 'lucide-react';
...
export default function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const handleSaveMember = async (memberData: Omit<Member, 'id'>) => {
    ...
    if (editingMember) {
      ...
      const { error } = await supabase.from('members').insert([dbData]);
      if (!error) fetchMembers();
    }
  ...
  // Función para confirmar la eliminación definitiva
  const confirmDelete = async () => {
    const memberToDelete = deleteConfirmation.member;
    if (memberToDelete) {
  ...
  // Obtenemos todos los nombres completos para el 'predictivo' del modal
  const allMembersNames = members.map(m => `${m.firstName} ${m.lastName}`);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans pb-10">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-white text-black px-6 py-2.5 rounded-xl font-black flex items-center gap-2 hover:bg-zinc-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.15)] active:scale-95"
            >
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-3xl">
            <div className="flex items-center gap-4">
              ...
              <div><p className="text-xs font-black text-zinc-500 uppercase">Total Alumnos</p><p className="text-2xl font-bold">{isLoading ? '...' : stats.total}</p></div>
            </div>
          </div>
          <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-3xl">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-emerald-500/10 rounded-2xl text-emerald-500"><CalendarClock size={24}/></div>
              <div><p className="text-xs font-black text-zinc-500 uppercase">Activos</p><p className="text-2xl font-bold text-emerald-400">{isLoading ? '...' : stats.active}</p></div>
            </div>
          </div>
          <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-3xl">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-500"><AlertTriangle size={24}/></div>
              <div><p className="text-xs font-black text-zinc-500 uppercase">Vence en 1 Mes</p><p className="text-2xl font-bold text-blue-400">{isLoading ? '...' : stats.expiringSoon}</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 items-center justify-between">
          <div className="flex gap-2 bg-zinc-900/80 p-1.5 rounded-2xl border border-zinc-800 shadow-inner">
            <button onClick={() => setFilter('all')} className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all ${filter === 'all' ? 'bg-white text-black' : 'text-zinc-500 hover:text-white'}`}>Todos</button>
            <button onClick={() => setFilter('nextMonth')} className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all ${filter === 'nextMonth' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-white'}`}>Vence en 1 mes</button>
          </div>
          <div className="relative w-full md:w-96 group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 transition-colors" size={18}/>
            <input 
              type="text" placeholder="Buscar por nombre o DNI..." 
              className="w-full bg-zinc-900/50 border border-zinc-800 rounded-2xl pl-12 pr-4 py-3.5 focus:border-zinc-500 transition-all outline-none"
      ...
      <MemberModal 
        ...
        allMembersNames={allMembersNames} // Pasamos los nombres para el predictivo
      />

      {/* DIÁLOGO DE CONFIRMACIÓN DE ELIMINACIÓN (ALERTA) */}
      <AnimatePresence>
              ...
              <div className="flex flex-col items-center gap-6 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-400 mt-2">
                    Estás a punto de eliminar la matrícula de <strong className="text-white">{deleteConfirmation.member?.firstName} {deleteConfirmation.member?.lastName}</strong>. Esta acción no se puede deshacer.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full mt-4">
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-red-500 text-white font-black uppercase text-xs rounded-xl hover:bg-red-600 transition-all shadow-[0_0_20px_rgba(239,68,68,0.2)] active:scale-95 flex items-center justify-center gap-2"
                  >
                    <Trash2 size={16}/>
                    Sí, Eliminar
                  </button>
